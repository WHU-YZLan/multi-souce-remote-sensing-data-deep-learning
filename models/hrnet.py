...
def fuse_layer3(x):
    x0_0 = x[0]
    # x0_0 = UpSampling2D(size=(4, 4))(x0_0)
    x0_1 = Conv2D(32, 1, use_bias=False, kernel_initializer='he_normal')(x[1])
    x0_1 = BatchNormalization(axis=3)(x0_1)
    x0_1 = UpSampling2D(size=(2, 2))(x0_1)
    x0_2 = Conv2D(32, 1, use_bias=False, kernel_initializer='he_normal')(x[2])
...
def final_layer(x, n_classes=17):
    x = Conv2D(n_classes, 1, use_bias=False, kernel_initializer='he_normal')(x)

    return x


def seg_hrnet(input_height=256, input_width=256, channels=3, n_classes=17):
    ...
    out = final_layer(x, n_classes)

    # model = Model(inputs=inputs, outputs=out)
    model = get_segmentation_model(inputs, out)
    model.model_name = "hrnet"
